...
public class SavedCharacter {
    ...
    public static function getImage(savedChar:SavedCharacter, playerXML:XML, dir:int, action:int, p:Number, available:Boolean, selected:Boolean):BitmapData {
        ...
        var tex1:int = savedChar != null ? int(int(savedChar.tex1())) : int(int(null));
        var tex2:int = savedChar != null ? int(int(savedChar.tex2())) : int(int(null));
    ...
    public static function compare(char1:SavedCharacter, char2:SavedCharacter):Number {
        var char1Use:Number = !!Boolean(Parameters.data.charIdUseMap.hasOwnProperty(char1.charId().toString())) ? Number(Number(Parameters.data.charIdUseMap[char1.charId()])) : Number(Number(0));
        var char2Use:Number = !!Boolean(Parameters.data.charIdUseMap.hasOwnProperty(char2.charId().toString())) ? Number(Number(Parameters.data.charIdUseMap[char2.charId()])) : Number(Number(0));
