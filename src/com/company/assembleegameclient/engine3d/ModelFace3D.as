...
public class ModelFace3D {
    ...
    public static function compare(f1:ModelFace3D, f2:ModelFace3D):Number {
        ...
        for (i = 0; i < f1.indicies_.length; i++) {
            ...
            minZ1 = val < minZ1 ? Number(Number(val)) : Number(Number(minZ1));
            maxZ1 = val > maxZ1 ? Number(Number(val)) : Number(Number(maxZ1));
        }
        ...
        for (i = 0; i < f2.indicies_.length; i++) {
            ...
            minZ2 = val < minZ2 ? Number(Number(val)) : Number(Number(minZ2));
            maxZ2 = val > maxZ2 ? Number(Number(val)) : Number(Number(maxZ2));
        }
        if (minZ2 > minZ1) {
            return -1;
        }
