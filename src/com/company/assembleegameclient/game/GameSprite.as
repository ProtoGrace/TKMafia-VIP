...
public class GameSprite extends Sprite {
    ...
    public function initialize():void {
        ...
        if (Parameters.data.autoGodmode)
            Parameters.data.godmode = true;
        if (Parameters.data.autoSMult)
    ...
    public function connect():void {
        if (!this.isGameStarted) {
            ...
            if (Parameters.data.FS) {
                if (Parameters.data.mscale == undefined) {
                    Parameters.data.mscale = "1.0";
                }
                if (Parameters.data.stageScale == undefined) {
                    Parameters.data.stageScale = StageScaleMode.NO_SCALE;
                }
                if (Parameters.data.uiscale == undefined) {
                    Parameters.data.uiscale = true;
                }
                Parameters.save();
                stage.scaleMode = Parameters.data.stageScale;
    ...
    public function DamageCounter():void {
        var name:* = undefined;
        var dmgInflicted:* = undefined;
        var gameObject:GameObject = null;
        var objectId:int = 0;
        for each(gameObject in this.map.goDict_) {
            ...
            if ((gameObject.props_.isQuest_ || gameObject.props_.isChest_) && Parameters.DamageCounter[gameObject.objectId_] > 0) {
                if (gameObject != null) {
                    name = ObjectLibrary.typeToDisplayId_[gameObject.objectType_];
                    ...
                    dmgInflicted = int(Parameters.DamageCounter[gameObject.objectId_] / gameObject.maxHP_ * 100);
    ...
    public function onScreenResize(_arg_1:Event):void {
        ...
        if (this.map != null) {
            this.map.scaleX = sWidth * Number(Parameters.data.mscale);
            this.map.scaleY = sHeight * Number(Parameters.data.mscale);
        }
    ...
    private function onEnterFrame(event:Event):void {
        ...
        var player:Player = this.map.player_;
        if (this.focus) {
            this.camera_.configureCamera(this.focus, !!Boolean(player) ? Boolean(Boolean(player.isHallucinating())) : Boolean(Boolean(false)));
        ...
        if (Parameters.data.autoDrink) {
            var consumableLen:int = this.map.consumableContainers.length;
            for (var i:int = 0; i < consumableLen; i++) {
                var go:GameObject = this.map.consumableContainers[i];
                if (PointUtil.distanceSquaredXY(go.x_, go.y_, this.map.player_.x_, this.map.player_.y_) <= 1) {
                    if (!go.equipment_)
                        continue;

                    var equipLen:int = go.equipment_.length;
                    for (var j:int = 0; j < equipLen; j++) {
                        var itemId:int = go.equipment_[j];
                        var xml:XML = ObjectLibrary.xmlLibrary_[itemId];

                        if (xml && (xml.hasOwnProperty("Consumable")
                            ...
                            if (Parameters.data.consumeNormal
                                    || !xml.hasOwnProperty("Activate")
                                    || xml.Activate != "IncrementStat")
                                this.gsc_.useItem_new(go, go.equipment_.indexOf(itemId));
                        }
                    }
                }
            }
