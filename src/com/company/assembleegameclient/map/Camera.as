...
public class Camera {
    ...
    public static function adjustDimensions():void {
        var mscale:Number = Parameters.data.mscale;
        var scaleW:Number = WebMain.sWidth / mscale;
        var scaleH:Number = WebMain.sHeight / mscale;
        var p:Number = Number(scaleH / 3);
        CenterRect = new Rectangle((p - scaleW) / 2, -scaleH * 13 / 24, scaleW, scaleH);
        OffCenterRect = new Rectangle((p - scaleW) / 2, -scaleH * 3 / 4, scaleW, scaleH);
    }
    ...
    public function configureCamera(object:GameObject, isHallucinating:Boolean):void {
        var screenRect:Rectangle = this.correctViewingArea(Parameters.data.centerOnPlayer);
        var cameraAngle:Number = Parameters.data.cameraAngle;
    ...
    public function correctViewingArea(param1:Boolean) : Rectangle {
        ...
        if(Parameters.data.FS) {
            _loc3_ = WebMain.STAGE.stageWidth / Parameters.data.mscale;
            _loc4_ = WebMain.STAGE.stageHeight / Parameters.data.mscale;
            _loc2_ = 200 * WebMain.STAGE.stageWidth / 800;
    ...
    public function configure(x:Number, y:Number, z:Number, angleRad:Number, clipRect:Rectangle, usePP:Boolean):void {
        ...
        this.rd_[9] = !!usePP ? Number(Number(this.u_.z)) : Number(Number(-1));
