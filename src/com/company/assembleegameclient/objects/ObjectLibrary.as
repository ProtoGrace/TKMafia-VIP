...
public class ObjectLibrary {
    ...
    public static function getRedrawnTextureFromType(objectType:int, size:int, includeBottom:Boolean, useCaching:Boolean = true, scaleValue:int = 5):BitmapData {
        ...
        var tex1:int = Boolean(objectXML.hasOwnProperty("Tex1")) ? int(int(int(objectXML.Tex1))) : int(int(0));
        var tex2:int = Boolean(objectXML.hasOwnProperty("Tex2")) ? int(int(int(objectXML.Tex2))) : int(int(0));
    ...
    public static function isSoulbound(objectType:int):Boolean {
        ...
        return objectXML != null && (objectXML.hasOwnProperty("Soulbound") || objectXML.hasOwnProperty("Tier") && isType(slotType) && tier < 4 || objectXML.hasOwnProperty("Tier") && isTypeWeapon(slotType) && tier < 11 || objectXML.hasOwnProperty("Tier") && isTypeArmor(slotType) && tier < 11);
    }

    public static function isTypeArmor(slotType:int) : int {
        return int(slotType == ItemConstants.LEATHER_TYPE || slotType == ItemConstants.ROBE_TYPE || slotType == ItemConstants.PLATE_TYPE);
    }

    public static function isTypeWeapon(slotType:int):int {
        return int(slotType == ItemConstants.BOW_TYPE || slotType == ItemConstants.DAGGER_TYPE || slotType == ItemConstants.KATANA_TYPE || slotType == ItemConstants.STAFF_TYPE || slotType == ItemConstants.SWORD_TYPE || slotType == ItemConstants.WAND_TYPE);
    }

    public static function isType(slotType:int):int {
        return int(slotType == ItemConstants.RING_TYPE || slotType == ItemConstants.SHURIKEN_TYPE || slotType == ItemConstants.CLOAK_TYPE || slotType == ItemConstants.HELM_TYPE || slotType == ItemConstants.ORB_TYPE || slotType == ItemConstants.POISON_TYPE || slotType == ItemConstants.PRISM_TYPE || slotType == ItemConstants.TRAP_TYPE || slotType == ItemConstants.QUIVER_TYPE || slotType == ItemConstants.SCEPTER_TYPE || slotType == ItemConstants.SEAL_TYPE || slotType == ItemConstants.SHIELD_TYPE || slotType == ItemConstants.SPELL_TYPE || slotType == ItemConstants.SKULL_TYPE);
    }
