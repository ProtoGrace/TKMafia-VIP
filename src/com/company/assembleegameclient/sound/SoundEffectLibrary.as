...
public class SoundEffectLibrary {
    ...
    public static function play(name:String, volume:Number = 1.0, isFX:Boolean = true):void {
        var playFX:Boolean = Parameters.data.playSFX && isFX || !isFX && Parameters.data.playPewPew;
        if (!playFX) {
            return;
        }
    ...
    public static function updateTransform():void {
        ...
        for each(channel in activeSfxList_) {
            ...
            transform.volume = !!Boolean(Parameters.data.playSFX) ? Number(Number(activeSfxList_[channel])) : Number(Number(0));
