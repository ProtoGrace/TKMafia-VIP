...
public class TextBox extends Sprite {
    ...
    public function addText(name:String, text:String):void {
        var textBoxLine:TextBoxLine = new TextBoxLine(getTimer(), name, -1, -1, "", false, text, this._nameColor, this._textColor);
    ...
    public function addTextFull(name:String, objectId:int, numStars:int, recipient:String, text:String, nameColor:int, textColor:int):void {
        ...
        var textBoxLine:TextBoxLine = new TextBoxLine(getTimer(), name, objectId, numStars, recipient, toMe, text, nameColor, textColor);
        lines_.push(textBoxLine);
        this.refreshStatusBox();
        if (toMe) {
        ...
        } else if (recipient != Parameters.GUILD_CHAT_NAME || recipient != Parameters.PARTY_CHAT_NAME) {
            if (fromMe) {
    ...
    public function insertTellPlayerPrefix(name:String):void {
        if (stage.focus == null) {
            this.selectInput();
        }
        this.inputField_.text = "/tell " + name + " ";
    ...
    private function addTextBlock(textBlock:TextBlock):void {
        ...
        var yPos:Number = 0;
        while (textLine = textBlock.createTextLine(textLine, yPos == 0 ? Number(Number(this.w_ - 4)) : Number(Number(this.w_ - 4 - INDENT)))) {
            textLine.x = yPos == 0 ? Number(Number(0)) : Number(Number(INDENT));
    ...
    private function onKeyUp(event:KeyboardEvent):void {
        switch (event.keyCode) {
            case Parameters.data.chatCommand:
                if (stage.focus == null) {
                    this.selectInput();
                    this.inputField_.text = "/";
                    this.inputField_.setSelection(this.inputField_.length, this.inputField_.length);
                ...
                break;
            case Parameters.data.chat:
                if (stage.focus == null) {
                    this.selectInput();
                }
                break;
            case Parameters.data.tell:
                if (stage.focus == null) {
                    this.selectInput();
                }
                this.insertTellPrefix(event.shiftKey);
                break;
            case Parameters.data.guildChat:
                if (stage.focus == null) {
                    this.selectInput();
                ...
                break;
            case Parameters.data.partyChat:
                if(stage.focus == null) {
                    this.selectInput();
                    this.inputField_.text = "/p ";
                    this.inputField_.setSelection(this.inputField_.length,this.inputField_.length);
                }
                break;
            case Parameters.data.scrollChatUp:
                if (!this.showMax_) {
                    this.showMax_ = true;
                } else {
                ...
                this.refreshStatusBox();
                break;
            case Parameters.data.scrollChatDown:
                if (this.lc_ == 0) {
                    this.showMax_ = false;
                } else {
