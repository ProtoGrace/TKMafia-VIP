...
public class BitmapUtil {
    public static function trimAlpha(param1:BitmapData) : BitmapData {
    ...
    public static function lineOfSight(bitmapData:BitmapData, p1:IntPoint, p2:IntPoint):Boolean {
        ...
        var deltay:int = y0 > y1 ? int(int(y0 - y1)) : int(int(y1 - y0));
        var error:int = -(deltax + 1) / 2;
        var ystep:int = y0 > y1 ? int(int(-1)) : int(int(1));
        var xstop:int = x1 > width - 1 ? int(int(width - 1)) : int(int(x1));
        var y:int = y0;
        var x:int = x0;
        if (x < 0) {
        ...
        if (ystep > 0 && y < 0 || ystep < 0 && y >= height) {
            skipYSteps = ystep > 0 ? int(int(-y - 1)) : int(int(y - height));
