...
public class BuyCharacterSlotCommand {
    ...
    private function isSlotUnaffordable():Boolean {
        return (!!this.model.isNextCharSlotCurrencyFame() ? this.model.getFame() : this.model.getCredits()) < this.model.getNextCharSlotPrice();
    }

    private function nonSufficientFunds():void {
        this.openDialog.dispatch(new FlexibleDialog("Not Enough " + (!!this.model.isNextCharSlotCurrencyFame() ? "Fame" : "Gold"), "Insufficient funds when trying to buy a slot.", "Close"));
    }
