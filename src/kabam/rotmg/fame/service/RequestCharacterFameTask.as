...
public class RequestCharacterFameTask extends BaseTask {
    ...
    private function parseXML():void {
        ...
        this.texture1 = !!Boolean(charXml.hasOwnProperty("Tex1")) ? int(int(charXml.Tex1)) : int(int(0));
        this.texture2 = !!Boolean(charXml.hasOwnProperty("Tex2")) ? int(int(charXml.Tex2)) : int(int(0));
    }

    private function getDeathDate():String {
        var time:Number = Number(this.xml.CreatedOn) * 1000;
