...
public class PlayGameCommand {
    ...
    private function makeGameView():void {
        ...
        var gameId:int = !!this.data.isNewGame ? int(int(this.getInitialGameId())) : int(int(this.data.gameId));
        var createCharacter:Boolean = this.data.createCharacter;
        var charId:int = this.data.charId;
        var keyTime:int = !!this.data.isNewGame ? int(int(-1)) : int(int(this.data.keyTime));
